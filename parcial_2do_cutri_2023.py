...
def ind_nesima_aparición(s:list[int], n:int, elem:int) -> int:
    cantidad_de_apariciones_parciales:int = 0
    posicion_elem_repetido_n_veces:int = 0

    for i in range(len(s)):
        if s[i] == elem:
            cantidad_de_apariciones_parciales += 1
            if cantidad_de_apariciones_parciales == n:
                posicion_elem_repetido_n_veces = i

    if cantidad_de_apariciones_parciales < n:
        posicion_elem_repetido_n_veces = -1
    
    return posicion_elem_repetido_n_veces

print(ind_nesima_aparición([-1, 1, 2, 5, -7, 2, 1],2,1))
...
def mezclar(s1:list[int],s2:list[int]) -> list[int]:
    listas_sumadas: list[int] = s1 + s2
    listas_mezcladas : list[int] = []
    i:int = 0
    for i in range(len(s1)):
        listas_mezcladas.append(s1[i])
        listas_mezcladas.append(s2[i])
    return listas_mezcladas

print(mezclar([2, 3],[4, 0]))
...
def genero_diccionario_con_caballos(caballos) -> dict[str,list[int]]:
    resultado: dict[str,list[int]] = dict() 
...
def frecuencia_posiciones_por_caballo(caballos: list[str],carreras:dict[str,list[str]]) -> dict[str,list[int]]:
    ...
    listado_carreras: list[str] = list(carreras.keys())
    
    resultado = genero_diccionario_con_caballos(caballos)

    for carrera in listado_carreras:
        resultados_carrera: list[str] = carreras[carrera]
        for posicion in range(len(resultados_carrera)):
            resultado[resultados_carrera[posicion]][posicion] += 1
...
def matriz_capicua(m:list[list[int]]) -> bool:
    contador_booleanos: int = 0
    for fila in m:
        if fila_capicua(fila):
            contador_booleanos +=1
    
    return len(m) == contador_booleanos
